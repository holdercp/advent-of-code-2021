...
with open('23/input.txt') as f:
    ...
    for i, l in enumerate(lines):
        ...
        if i == 3:
            initial_state['A4'] = l[3]
            initial_state['B4'] = l[5]
            initial_state['C4'] = l[7]
            initial_state['D4'] = l[9]
    initial_state['A2'] = 'D'
    initial_state['B2'] = 'C'
    initial_state['C2'] = 'B'
    initial_state['D2'] = 'A'
...
def should_move(pod, loc, state):
    ...
    if loc[1] == '1':
        if (f'{loc[0]}2' in state and state[f'{loc[0]}2'] != pod) or (f'{loc[0]}3' in state and state[f'{loc[0]}3'] != pod) or (f'{loc[0]}4' in state and state[f'{loc[0]}4'] != pod):
            return True
    if loc[1] == '2':
        if (f'{loc[0]}3' in state and state[f'{loc[0]}3'] != pod) or (f'{loc[0]}4' in state and state[f'{loc[0]}4'] != pod):
            return True
    if loc[1] == '3':
        if f'{loc[0]}4' in state and state[f'{loc[0]}4'] != pod:
            return True
...
def find_room_moves(pod, loc, state, graph, energy_count=0, visited=[]):
    ...
    for n in neighbors:
        if n[0] == pod:
            ...
            if n[1] == '3' and (f'{n[0]}4' in state and state[f'{n[0]}4'] == pod):
                room_moves.append((energy_count + 1, n))
            if n[1] == '2' and (f'{n[0]}3' in state and state[f'{n[0]}3'] == pod) and (f'{n[0]}4' in state and state[f'{n[0]}4'] == pod):
                room_moves.append((energy_count + 1, n))
            if n[1] == '1' and (f'{n[0]}2' in state and state[f'{n[0]}2'] == pod) and (f'{n[0]}3' in state and state[f'{n[0]}3'] == pod) and (f'{n[0]}4' in state and state[f'{n[0]}4'] == pod):
                room_moves.append((energy_count + 1, n))
        room_moves += find_room_moves(pod, n, state,
                                      graph, energy_count + 1, visited)
    return room_moves
...
def org_pods(graph, initial_state):
    target = (('A1', 'A'), ('A2', 'A'), ('A3', 'A'), ('A4', 'A'), ('B1', 'B'),
              ('B2', 'B'), ('B3', 'B'), ('B4', 'B'), ('C1', 'C'), ('C2', 'C'), ('C3', 'C'), ('C4', 'C'), ('D1', 'D'), ('D2', 'D'), ('D3', 'D'), ('D4', 'D'))
    q = [(0, initial_state)]
    states = {}
